...
if (isset($_POST['submit'])) {
    $password = $_POST['pass'];
    $hashedPassword = password_hash($password, PASSWORD_DEFAULT); // Hash the password only once
    ...
    if ($admin && password_verify($opassword, $admin['password'])) {
        if (password_verify($cpassword, $hashedPassword)) {
            $update_pass = $conn->prepare("UPDATE `admin` SET password = ? WHERE id = ?");
            $update_pass->execute([$hashedPassword, $admin_id]);
        };
    ...
    <div class="adminc">
        ...
        <form method="post">
            <div class="form-group">
                ...
                <input type="Password" name="opass" required placeholder="Enter Your Password" />
            </div>
            <div class="form-group">
            ...
            <div class="form-group">
                ...
                <input type="Password" name="cpass" required placeholder="Enter Your Password" />
            </div>
            <div class="form-group">
                <input type="submit" name="submit" value="SUBMIT" required class="submit" />
            </div>
        </form>
    </div>
</body>
